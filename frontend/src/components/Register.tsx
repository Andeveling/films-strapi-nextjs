...
const Register = () => {
  ...
  const handleSubmit = async (e: SyntheticEvent<HTMLFormElement>) => {
    e.preventDefault()
    try {
      const responseData = await fetcher(`${server}/auth/local/register`, {
        headers: {
  ...
  return (
    <div className='flex w-full'>
      <div className='w-full bg-white rounded p-8  md:max-w-sm md:mx-auto'>
        ...
        <form onSubmit={handleSubmit} className='mb-4 md:flex md:flex-wrap md:justify-between'>
          ...
          <button className='block bg-teal-400 text-lg rounded p-4 mx-auto' type='submit'>
            Register
          </button>
        </form>
      </div>
